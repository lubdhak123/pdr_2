"""
MSME Synthetic Data Generator v4 — Score-Driven Labels
=======================================================
Generate features from a single realistic distribution per business type.
Assign default label using a weighted logistic function of the features.
This guarantees natural overlap and realistic AUC range of 0.80-0.92.

Run: python scripts/generate_msme_data.py
"""
import os
import numpy as np
import pandas as pd
...
# regardless of whether they default or not.
# Default is assigned AFTER based on a scoring function.
...
def assign_defaults(df, default_rate):
    ...
    print(f"raw_score range: {raw_score.min():.2f} to {raw_score.max():.2f}")

    # Standardize to mean=0 std=1 so binary search always works
    score = (raw_score - raw_score.mean()) / (raw_score.std() + 1e-9)

    # Add noise AFTER standardizing so signal-to-noise ratio is controlled
    score = score + np.random.normal(0, 0.8, len(df))

    # Binary search for intercept over wide bounds
    lo_int, hi_int = -10.0, 10.0
    for _ in range(100):
        mid = (lo_int + hi_int) / 2
        proba = expit(score - mid)
        if proba.mean() > default_rate:
            lo_int = mid
        else:
            hi_int = mid
    ...
    print(f"    Calibrated default rate: {actual_rate:.3f} (target: {default_rate:.3f})")
    assert abs(actual_rate - default_rate) < 0.05, \
        f"Calibration failed: got {actual_rate:.3f}, wanted {default_rate:.3f}"

    return np.random.binomial(1, proba), proba
...
def build_type(biz_type, n):
    ...
    defaults, proba = assign_defaults(df, p["default_rate"])
    df["default"] = defaults
    return df

print("Generating score-driven MSME synthetic data...\n")
all_frames = []
for biz_type in TYPE_PARAMS:
    ...
    print(f"  {biz_type:<22} → {len(df_type)} rows | "
          f"Actual default rate: {df_type['default'].mean():.1%}")
...
out = os.path.join(DATA_DIR, "msme_synthetic.csv")
